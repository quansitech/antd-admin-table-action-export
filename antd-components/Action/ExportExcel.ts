...
type This = {
    ...
    execCount: number,
    progressNum: number,
    cellPropertiesKey: any,
    cellPropertiesValues: any
}

function ExportExcel(this: This, opt: any) {
    this.wopts = {
        bookType: 'xlsx',
        bookSST: false,
        type: 'binary',
...
ExportExcel.prototype.generateExcelBlob = function () {
    var buffer = this.s2ab(writeXLSX(this.wb, this.wopts));
    return new Blob([buffer], {type: "application/octet-stream"});
}

ExportExcel.prototype.pushSheet = function (data, sheetName) {
    this.wb.SheetNames.push(sheetName);
    this.wb.Sheets[sheetName] = utils.json_to_sheet(this.filterCellProperties(data));
    this.wb.Sheets[sheetName]['!merges'] = this.genMerges(this);
    this.execCount--;
    this.execCount === 0 && this.makeExcel();
}

ExportExcel.prototype.filterCellProperties = function (data: any) {
    let obj = this;
    let newData = [];
